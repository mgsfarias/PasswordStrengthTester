...
def analisar_senha(event=None):
    ...
    mensagem_vazamento = verificar_vazamento(senha)
    texto_vazamento.set(mensagem_vazamento)
...
#Verificar a senha com HaveIBeenPwned
def verificar_vazamento(senha):
    ...
    for linha in resposta.text.splitlines():
        ...
        if hash_sufixo == sufixo:
            return f"⚠ Esta senha já vazou {contagem} vezes!"

    return "✅ Esta senha não foi encontrada em vazamentos públicos."

# Resultado
tb.Label(tema, textvariable=texto_tempo, font=("Arial", 10), foreground="red").pack(pady=10)
texto_vazamento = tk.StringVar()
tb.Label(tema, textvariable=texto_vazamento, font=("Arial", 10), foreground="orange").pack(pady=(0, 10))
